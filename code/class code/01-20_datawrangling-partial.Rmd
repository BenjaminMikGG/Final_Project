---
title: "02 Data wrangling"
---

# 1) Setup  
## Setup 2
```{r Setup, message=FALSE}
# Loading packages (every time you open R)
library(readxl) # to read excel files
library(janitor) # to clean data
library(dplyr) # wranglingdd
library(tidyr) # wrangling
library(readr) # to export csv

```

```{r}
# r means the language here. Here is R, it can by python.
```

```{r}
getwd()
```

ˆ
```{r Reading data}
# inside the "" cursor. you can tab tab button to see where you are, which dir you are in.
# use .. to go back to upper level, / to go lower level.
soyk <- read_excel("../data/01_soybeans.xlsx")

# Checking first rows
soyk

# View
View(soyk)
```

This study was conducted at four different locations in one year to assess the effect of potassium (K) fertilizer rates on soybean trifoliate K concentration at growth stage R4.  

Each study was a randomized complete block design (RCBD) with K rate as the only treatment factor with levels 0, 50, 100, and 150 lbs K2O/ac.

```{r Glimpse}
glimpse(soyk)
```

```{r Summary}
summary(soyk)
```

# 2) Data wrangling  
```{r soyk_1 - clean_names()}
soyk_1 <- clean_names(soyk)

soyk_1
```


```{r soyk_2 - filter()}
names(soyk_1)

soyk_2 <- filter(soyk_1,
                 k_rate_lbac == 0 & plot == 105
                 )

soyk_2
```

```{r soyk_3 - mutate()}
# Transforming K rate from lbs/ac to kg/ha
soyk_3  <- mutate(soyk_1,
                  k_rate_kgha = k_rate_lbac * 0.454/0.405,
                  k_rate_kgha = round(k_rate_kgha, 0)
                  )

soyk_3
```

```{r soyk_4 - case_when()}
soyk_4 <- mutate(soyk_3, 
                 rep = case_when(
                   plot < 200 ~ 1,
                   plot >200 & plot<300 ~ 2,
                   plot >300 & plot<400 ~ 3,
                   plot >400 ~ 4
                   
                 )
                 )

soyk_4

```

```{r soyk_5 - pivot_longer()}
names(soyk_4)

soyk_5 <- pivot_longer(soyk_4, 
                       names_to = "location.k",
                       values_to = "k_pct",
                       cols = contains("k_pct")
                       )

soyk_5

View(soyk_5)
```

```{r soyk_6 - separate()}
soyk_6 <- separate(soyk_5,
                   location.k,
                   into = c("location", "k","pct"),
                   sep="_"
                   )

soyk_6

View(soyk_6)
```

```{r soyk_7 - select()}
# select is for columns, filter is for rows
names(soyk_6)
# select function is also common from other package, so we put dplyr:: here to specify the pack name.
soyk_7 <- dplyr::select(soyk_6,
                 location, rep, k_rate_kgha, k_pct
                 )

soyk_7
```

```{r soyk_8 - arrange()}
soyk_8 <- arrange(soyk_7,
                  #location
                  rep
                  )

soyk_8
```

```{r soyk_9 and 10 - group_by and summarise}
# group_by
soyk_9 <- group_by(soyk_8,
                   location
                   )

soyk_9 

class(soyk_8)

class(soyk_9)

# summarise
soyk_10  <- summarise(soyk_9,
                      # lot of other stat functions like max, min, std and so on.
                      mean_pct = mean(k_pct)
                      
                      )

soyk_10
```

```{r Piping it all together}
soyk_w <- soyk %>%
  # or |> will do the similar job
  clean_names() %>%
  mutate(
                  k_rate_kgha = k_rate_lbac * 0.454/0.405,
                  k_rate_kgha = round(k_rate_kgha, 0)
                  ) %>%
  mutate( 
                 rep = case_when(
                   plot < 200 ~ 1,
                   plot >200 & plot<300 ~ 2,
                   plot >300 & plot<400 ~ 3,
                   plot >400 ~ 4
                   
                 )
                 ) %>%
  pivot_longer( 
                       names_to = "location.k",
                       values_to = "k_pct",
                       cols = contains("k_pct")
                       ) %>%
  separate( 
                   location.k,
                   into = c("location", "k","pct"),
                   sep="_"
                   ) %>%
  dplyr::select(
                 location, rep, k_rate_kgha, k_pct
                 ) %>%
  arrange( 
                  #location
                  rep
                  )

soyk_w


```

```{r Checking if same as without pipe}
# Same result?
identical(soyk_8, soyk_w)

```

```{r Saving to file for further analysis}
write_csv(soyk_w,
          "../output/soybean_processed.csv"
          )

```

# 3) Assignment   
Starting from soyk, use data wrangling functions in a pipe workflow to generate the average k_pct for each K rate (in kg/ha) within each location (basically, averaging over reps). You can use the chunk below to complete this quiz.

```{r assignment}
soyk_ass2 <- soyk %>%
  clean_names() %>%
  mutate(
                  k_rate_kgha = k_rate_lbac * 0.454/0.405,
                  k_rate_kgha = round(k_rate_kgha, 0)
                  ) %>%
  mutate( 
                 rep = case_when(
                   plot < 200 ~ 1,
                   plot >200 & plot<300 ~ 2,
                   plot >300 & plot<400 ~ 3,
                   plot >400 ~ 4
                   
                 )
                 ) %>%
  pivot_longer( 
                       names_to = "location.k",
                       values_to = "k_pct",
                       cols = contains("k_pct")
                       ) %>%
  separate( 
                   location.k,
                   into = c("location", "k","pct"),
                   sep="_"
                   ) %>%
  dplyr::select(
                 location, rep, k_rate_kgha, k_pct
                 ) %>%
  arrange( 
                  #location
                  rep
                  ) %>%
  group_by(
                   location,
                   k_rate_kgha
                   ) %>%
 
 summarise( 
                      mean_pct = mean(k_pct)
                      
                      )


soyk_ass2


```
soyk_ass2
```


Tip: the final data frame will have 16 rows.

Submit your result (take a screenshot that includes the code and the chunk output) on eLC under Assignment #2 by Monday Jan 26th 11:59 pm.



